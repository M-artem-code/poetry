...
import { useComments, useCreateComment, useDeleteComment } from '@/src/features/comments'
import { useCheckFavorite, useToggleFavorite } from '@/src/features/favorites'
...
const PoemViewSection = ({ poemId }: PoemViewSectionProps) => {
  ...
  // Комментарии
  const { data: comments = [], isLoading: isLoadingComments } = useComments(poemIdNum)
  const createComment = useCreateComment()
  const deleteComment = useDeleteComment(poemIdNum)
  
  // Избранное
  const { data: favoriteStatus } = useCheckFavorite(poemIdNum)
  ...
  const handleFavorite = () => {
    ...
    toggleFavorite.mutate({ poemId: poemIdNum, isFavorite })
  }
    ...
    return (
      <section className={styles.poemViewSection}>
        ...
        <div className="container">
          <div style={{ textAlign: 'center', padding: '100px 0', position: 'relative', zIndex: 2 }}>
            <p>Загрузка...</p>
          </div>
        </div>
      </section>
  ...
  const videoUrl = poem.videoUrl 
    ? (poem.videoUrl.startsWith('/') ? poem.videoUrl : `${API_URL}${poem.videoUrl}`)
    : null
      ...
      <div className="container">
        ...
        <div className={styles.poemContainer}>
          <div className={styles.poemHeader}>
            <p className={styles.authorLabel}>{authorName} {authorYears}</p>
          ...
          {poem.author?.bio && (
            <div className={styles.authorBio}>
              ...
              {poem.author.slug && (
                <Link href={`/author/${poem.author.slug}`} className={styles.bioLink}>
          ...
          <div className={styles.actions}>
            <div className={styles.actionsButtons}>
              ...
              >
                <svg className={styles.icon} viewBox="0 0 24 24" fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
          ...
          <div className={styles.commentsSection}>
            ...
            <div className={styles.commentsList}>
              {isLoadingComments ? (
                <p style={{ textAlign: 'center', color: '#999' }}>Загрузка каментарыяў...</p>
              ) : comments.length === 0 ? (
                <p style={{ textAlign: 'center', color: '#999' }}>Пакуль няма каментарыяў. Будзьце першым!</p>
              ) : (
                    ...
                    <div className={styles.commentContent}>
                      <div className={styles.commentHeader}>
                        <span className={styles.commentAuthor}>
                          {comment.user?.name || comment.user?.email || 'Карыстальнік'}
                        </span>
