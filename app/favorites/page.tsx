...
export default function FavoritesPage() {
  ...
  const handleRemoveFavorite = async (poemId: number) => {
    try {
      await removeFavorite.mutateAsync(poemId)
    } catch (err) {
    ...
    if (favorites.length === 0) {
      return (
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyText}>
            Начните добавлять стихотворения в избранное, чтобы они появились здесь
          </p>
          <Link href="/" className={styles.browseButton}>
            Посмотреть стихи
          </Link>
        ...
        {favorites.map((favorite) => (
          <div key={favorite.id} className={styles.card}>
            <div className={styles.cardContent}>
              <Link href={`/poem/${favorite.poem.id}`} className={styles.cardLink}>
                <h3 className={styles.cardTitle}>{favorite.poem.title}</h3>
                <div className={styles.cardMeta}>
                  <span className={styles.cardAuthor}>{favorite.poem.author?.name || 'Неизвестный автор'}</span>
                  {favorite.poem.year && (
                    <>
                      <span className={styles.cardDivider}>•</span>
                      <span className={styles.cardYear}>{favorite.poem.year}</span>
                    </>
                  )}
                </div>
                <p className={styles.cardExcerpt}>
                  {favorite.poem.content.split('\n').slice(0, 2).join('\n').substring(0, 120)}...
                </p>
              </Link>
              <button 
                className={styles.removeButton}
                onClick={() => handleRemoveFavorite(favorite.poem.id)}
