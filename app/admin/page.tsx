...
import type { AdminStats, AdminUser, CreatePoemDto } from '@/src/shared/types/admin.types'
...
export default function AdminPage() {
  ...
  const handleCreatePoem = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!poemForm.title || !poemForm.content || !poemForm.authorId || !poemForm.categoryId) {
    ...
    try {
      ...
      setEditingPoem(null)
      setPoemForm({ title: '', content: '', description: '', authorId: 0, year: undefined, categoryId: 0, videoUrl: '' })
      loadData()
    } catch (err: any) {
              ...
              <form className={styles.poemForm} onSubmit={handleCreatePoem}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <select
                    ...
                    onChange={(e) => setPoemForm(prev => ({ ...prev, authorId: parseInt(e.target.value) }))}
                    required
                  >
                ...
                <div className={styles.formGroup}>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setPoemForm(prev => ({ ...prev, description: e.target.value }))}
                    rows={4}
                ...
                <div className={styles.formGroup}>
                  ...
                  <input
                    type="number"
                    value={poemForm.year || ''}
                    onChange={(e) => setPoemForm(prev => ({ ...prev, year: e.target.value ? parseInt(e.target.value) : undefined }))}
                  />
                </div>
                ...
                <div className={styles.formGroup}>
                  ...
                  <select
                    ...
                    onChange={(e) => setPoemForm(prev => ({ ...prev, categoryId: parseInt(e.target.value) }))}
                    required
                  >
                ...
                <div className={styles.formGroup}>
                  ...
                  <div className={styles.videoUpload}>
                    ...
                    {poemForm.videoUrl && <span className={styles.videoPath}>{poemForm.videoUrl}</span>}
                  </div>
                </div>
              ...
              {poems.map(poem => (
                <div key={poem.id} className={styles.poemItem}>
                  <div className={styles.poemInfo}>
                    <h4>{poem.title}</h4>
                    <p>{poem.author?.name || 'Неизвестный автор'} {poem.year && `(${poem.year})`}</p>
                    <span className={styles.poemCategory}>
                      {poem.category?.name}
                    </span>
                    {poem.videoUrl && <span className={styles.hasVideo}>🎬 Видео</span>}
                  </div>
                  <div className={styles.poemActions}>
                    <button onClick={() => handleEditPoem(poem)}>✏️</button>
                    <button onClick={() => handleDeletePoem(poem.id)}>🗑️</button>
                  </div>
                </div>
              ))}
            </div>
              ...
              {users.map(u => (
                <div key={u.id} className={styles.userItem}>
                  <div className={styles.userItemInfo}>
                    <span className={styles.userItemName}>{u.name || 'Без имени'}</span>
                    <span className={styles.userItemEmail}>{u.email}</span>
                    <span className={`${styles.userItemRole} ${styles[u.role.toLowerCase()]}`}>
                      {u.role === 'SUPER_ADMIN' ? '👑 Суперадмин' : u.role === 'ADMIN' ? '⭐ Админ' : 'Пользователь'}
                    </span>
                  </div>
